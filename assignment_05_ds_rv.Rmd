...
- Prior to conducting the ANOVA, a QQ plot was created to evaluate normalcy of the weight distributions. The QQ plot revealed that the distributions of weights within each channel type is slightly abnormal. While this raises slight concern for the effectiveness of an ANOVA, it is widely accepted that an ANOVA is robust to violation of the normality assumption. Coupled with a Levene's test that indicates non-significant difference in variance, the results of the ANOVA test are still valuable. 

### **Summary**   
- 95% confidence interval for chi-squared test
- Surprising that there is little relationship between salamander and forest section type. Only indicates relationship between salamanders and clear-cutting, not other species
- 
