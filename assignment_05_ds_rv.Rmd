...
![Mack Creek Location](capture_01.PNG)
...
ggplot(data = mack_creek_count, aes(x = year, y = count, group = section_name))+
  ...
  theme_light()
```
**Figure 1**: *Counts of Pacific Giant Salamander’s compared between clear-cut forest section and old-growth forest section between 19993 and 2017. Visually, there is little difference between the counts of Pacific Giant Salamanders in clear cut and old growth forest sections.*
...
```
**Figure 1**: *Comparison of weights between Pacific Giant Salamanders observed in clear cut forest section and old growth forest section. Mean weight is displayed as black dot with error bars that display one standard deviation from mean. Mean weight of Pacific Giant Salamanders in clear cut forest sections = 7.78g, and old growth forest section = 6.58g.*
...
```
**Figure 5:** *QQ plot to explore normalcy of data spread for weights of Pacific Giant Salamanders observed in different channel classifications (Cascades, Pool, Side-Channel). Actual values of observations were plotted with theoretical normal values and yielded slightly abnormal results.*
